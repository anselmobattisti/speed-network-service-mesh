...
REF: https://github.com/networkservicemesh/deployments-k8s/tree/main/examples/interdomain/usecases/floating_nse_composition

This example show how to deploy an NSE composition 
...
The registry must be running in cluster3

```shell
kubectl get pods -n nsm-system --context=kind-cluster3

NAME                           READY   STATUS    RESTARTS   AGE
registry-k8s-5fc59bfbc-5g2cc   1/1     Running   0          99s
```
...
```
NAME                 AGE
interdomain-nse-composition   9m25s
```

To see the details of the NS

```shell
kubectl describe NetworkService -n ns-interdomain-nse-composition --context=kind-cluster3
```
...
Logs in the cluster passthrough

```shell
kubectl logs  nse-passthrough-3-5794f4ddd4-6sg4t -n ns-interdomain-nse-composition --context=kind-cluster2
```

Verificar o registro no cluster3
...
```shell
kubectl logs alpine -n ns-interdomain-nse-composition --context=kind-cluster1 -c cmd-nsc-init  --context=kind-cluster1
```
